...
/// <summary>
/// The main entry point for the Expense Tracker CLI application.
/// </summary>
internal class Program
{
    private static readonly string[] ListHeaders = ["Date", "Description", "Amount", "Category", "ID"];
    private static readonly string[] SummaryHeaders = ["Category", "Amount"];
    ...
    /// <summary>
    /// Creates the command for adding a new expense.
    /// </summary>
    /// <param name="service">The expense service.</param>
    ...
    private static Command CreateAddCommand(ExpenseService service)
    {
        ...
        command.SetAction(result =>
        {
            ...
            decimal amount = result.GetValue(amountOption);
            string category = result.GetValue(categoryOption)!;
            string dateStr = result.GetValue(dateOption)!;
    ...
    /// <summary>
    /// Creates the command for listing all expenses.
    /// </summary>
    /// <param name="service">The expense service.</param>
    /// <returns>A <see cref="Command"/> object.</returns>
    ...
    /// <summary>
    /// Creates the command for deleting an expense by ID.
    /// </summary>
    /// <param name="service">The expense service.</param>
    /// <returns>A <see cref="Command"/> object.</returns>
    private static Command CreateDeleteCommand(ExpenseService service)
    {
        ...
        command.SetAction(result =>
        {
            Guid id = result.GetValue(idOption);
    ...
    /// <summary>
    /// Creates the command for showing an expense summary.
    /// </summary>
    /// <param name="service">The expense service.</param>
    /// <returns>A <see cref="Command"/> object.</returns>
    private static Command CreateSummaryCommand(ExpenseService service)
    {
        ...
        command.SetAction(result =>
        {
            int? month = result.GetValue(monthOption);
            int? year = result.GetValue(yearOption);
            ...
            else
            {
                ...
                if (summary.Count > 0)
                {
                    ...
                    WriteTable(SummaryHeaders, rows);
                }
            }
        });
